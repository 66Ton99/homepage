...
const EN_URL = BASE . '/awg-to-amps';
const UK_URL = BASE . '/uk/awg-to-amps';
...
const DEFAULT_MATERIAL = 'cusn';


function material_json(string $lang): string
{
    $data = [];
    foreach (MATERIALS as [$key, $iacs, $en, $uk]) {
        $data[$key] = ['iacs' => $iacs, 'name' => $lang === 'en' ? $en : $uk];
    }
...
function material_options(string $lang): string
{
    $custom = $lang === 'en' ? 'Custom conductivity' : 'Власна провідність';
    $out = [];
    foreach (MATERIALS as [$key, $iacs, $en, $uk]) {
        $out[] = '                <option value="' . $key . '"'
            . ($key === DEFAULT_MATERIAL ? ' selected' : '') . '>'
            . ($lang === 'en' ? $en : $uk) . ' · ' . sprintf('%g', $iacs) . '% IACS</option>';
    }
...
function insulation_json(string $lang): string
{
    $data = [];
    foreach (INSULATIONS as [$key, $celsius, $en, $uk]) {
        $data[$key] = ['c' => $celsius, 'name' => $lang === 'en' ? $en : $uk];
    }
...
function insulation_options(string $lang): string
{
    $custom = $lang === 'en' ? 'Custom rating' : 'Власна температура';
    $out = [];
    foreach (INSULATIONS as [$key, $celsius, $en, $uk]) {
        $out[] = '                <option value="' . $key . '"'
            . ($key === 'sil200' ? ' selected' : '') . '>'
            . ($lang === 'en' ? $en : $uk) . ' · ' . $celsius . ' °C</option>';
    }
    $out[] = '                <option value="custom">' . $custom . '</option>';
...
const INSTALL_KEYS = ['bundle', 'free', '4-6', '7-9', '10-20', '21-30', '31-40', '41-plus'];


const COPPER_RESISTIVITY = 0.0175;  // Ω·mm²/m at 20 °C
const DEFAULT_VOLTAGE = ['dc' => 24, 'ac1' => 230, 'ac3' => 400];

/** The default conductor, mirrored from MATERIALS so the two cannot drift. */
function default_iacs(): float
{
    foreach (MATERIALS as [$key, $iacs]) {
        if ($key === DEFAULT_MATERIAL) {
            return $iacs;
        }
    }

    throw new RuntimeException('DEFAULT_MATERIAL ' . DEFAULT_MATERIAL . ' is not in MATERIALS');
}

define('DEFAULT_IACS', default_iacs());
define('DEFAULT_RESISTIVITY', COPPER_RESISTIVITY * (100 / DEFAULT_IACS));
define('DEFAULT_AMPACITY_FACTOR', (DEFAULT_IACS / 100) ** 0.5);


function fmt_area(float $area, string $decimalSep): string
{
    $text = $area < 10 ? sprintf('%.3f', $area) : sprintf('%.2f', $area);

    return str_replace('.', $decimalSep, $text);
}


function fmt_amps(float $amps, string $decimalSep): string
{
    return str_replace('.', $decimalSep, sprintf('%.1f', $amps));
}
 ...
 */
function fmt_resistance(float $milliohm, string $decimalSep): string
{
    $text = match (true) {
        $milliohm >= 100 => sprintf('%.0f', $milliohm),
        $milliohm >= 10 => sprintf('%.1f', $milliohm),
        $milliohm >= 1 => sprintf('%.2f', $milliohm),
        default => sprintf('%.3f', $milliohm),
    };
...
function fmt_length(float $metres, string $decimalSep): string
{
    $text = $metres >= 100 ? sprintf('%.0f', $metres) : sprintf('%.1f', $metres);
...
function fmt_power(float $watts, string $decimalSep, string $unitW, string $unitKw): string
{
    if ($watts < 1000) {
        return sprintf('%.0f', $watts) . ' ' . $unitW;
    }
    ...
    return str_replace('.', $decimalSep, sprintf('%.' . $digits . 'f', $kilowatts)) . ' ' . $unitKw;
}


/** Longest one-way run that keeps the drop within 3% at the rated current. */
function max_run_length(
    float $area,
    float $current,
    float $voltage,
    float $runFactor,
    ...
    return (0.03 * $voltage) / ($runFactor * $current * $resistance * $powerFactor);
}


function build_tbody(string $lang): string
{
    $sep = $lang === 'en' ? '.' : ',';
    $aria = $lang === 'en'
        ? 'Load {g} AWG into the calculator'
        : 'Підставити {g} AWG у калькулятор';
    $amp = $lang === 'en' ? 'A' : 'А';
    $watt = $lang === 'en' ? 'W' : 'Вт';
    $kilowatt = $lang === 'en' ? 'kW' : 'кВт';
    $out = [];
    ...
    foreach (ROWS as [$gauge, $area, $b60Ref, $f60Ref, $b200Ref, $f200Ref]) {
        [$b60, $f60, $b200, $f200] = array_map(
            static fn (int $value): float => $value * $factor,
            [$b60Ref, $f60Ref, $b200Ref, $f200Ref],
        );
        $resistance = fmt_resistance(DEFAULT_RESISTIVITY / $area * 1000, $sep);
        $length = fmt_length(max_run_length($area, $b60, DEFAULT_VOLTAGE['dc'], 2), $sep);
        $out[] = '                  <tr data-gauge="' . $gauge . "\">\n"
            . '                    <th scope="row"><button type="button" class="row-btn" '
            . 'aria-label="' . str_replace('{g}', $gauge, $aria) . '">' . $gauge
            . " <span class=\"unit\">AWG</span></button></th>\n"
            . '                    <td>' . fmt_area($area, $sep) . "</td>\n"
            . '                    <td class="js-b60">' . fmt_amps($b60, $sep) . ' ' . $amp . "</td>\n"
            . '                    <td class="js-f60">' . fmt_amps($f60, $sep) . ' ' . $amp . "</td>\n"
            . '                    <td class="js-b200">' . fmt_amps($b200, $sep) . ' ' . $amp . "</td>\n"
            . '                    <td class="js-f200">' . fmt_amps($f200, $sep) . ' ' . $amp . "</td>\n"
            . '                    <td class="js-r">' . $resistance . "</td>\n"
            . '                    <td class="js-len">' . $length . "</td>\n"
            . '                    <td class="js-p">'
            . fmt_power(DEFAULT_VOLTAGE['dc'] * $b60, $sep, $watt, $kilowatt) . "</td>\n"
            . '                  </tr>';
    }
...
function build_options(array $labels): string
{
    $out = [];
    foreach (INSTALL_KEYS as $key) {
        $out[] = '                      <option value="' . $key . '">' . $labels[$key] . '</option>';
    }
...
function faq_html(array $items): string
{
    $parts = ['            <div class="faq">'];
    foreach ($items as [$question, $answer]) {
        $parts[] = "              <details class=\"faq-item\">\n"
            . '                <summary class="faq-q"><h3>' . $question . "</h3></summary>\n"
...
function strip_tags_text(string $text): string
{
    // Collapse first, so what is left at the ends is a plain ASCII space and
    // trim() is enough. (*UCP) gives \s the Unicode reach Python's re has.
    return trim(preg_replace('/(*UCP)\s+/u', ' ', preg_replace('/<[^>]+>/u', '', $text)));
}


/** @param list<array{string,string}> $pairs */
function conv_html(array $pairs): string
{
    $body = [];
    foreach ($pairs as [$a, $b]) {
        $body[] = '              <div><dt>' . $a . '</dt><dd>' . $b . '</dd></div>';
    }

    return '            <dl class="conv-list">' . "\n" . implode("\n", $body) . "\n" . '            </dl>';
}


/** @param list<array{string,string}> $items */
function quicknav(array $items): string
{
    $out = [];
    foreach ($items as [$h, $t]) {
        $out[] = '              <li><a href="' . $h . '">' . $t . '</a></li>';
    }
...
$EN_FAQ = [
    [
        ...
        '<p>For tinned copper conductors at 30&nbsp;°C ambient, the conservative 60&nbsp;°C '
            . 'reference in the chart above gives roughly 7.8&nbsp;A for 18&nbsp;AWG, 13.7&nbsp;A for '
            . '14&nbsp;AWG, 25.5&nbsp;A for 10&nbsp;AWG, 34.3&nbsp;A for 8&nbsp;AWG, 60.7&nbsp;A for '
            . '4&nbsp;AWG and 109.7&nbsp;A for 0&nbsp;AWG with up to three current-carrying conductors in '
            . 'a cable. A single conductor in free air runs cooler and carries more. High-temperature '
            . 'silicone wire rated to 200&nbsp;°C carries roughly twice the conservative figure, but only '
            . 'if every terminal, fuse and connector in the circuit is rated for that temperature too.</p>',
    ],
    [
        ...
        '<p>Both describe the same copper. The difference is how hot you allow the conductor to get. '
            ...
            . 'insulation, terminals and adjacent materials are comfortable. The 200&nbsp;°C columns are '
            . 'the thermal limit of the silicone insulation itself — the current at which the conductor '
            . 'reaches 200&nbsp;°C. Treat the second number as a ceiling for short, well-ventilated runs, '
            . 'not as a design target.</p>',
    ],
    [
        'How do I convert AWG to mm²?',
        '<p>AWG is a logarithmic scale, so there is a closed formula rather than a simple ratio. The '
            . 'conductor diameter in millimetres is '
            . '<em>d</em>&nbsp;=&nbsp;0.127&nbsp;×&nbsp;92<sup>(36−AWG)/39</sup>, and the cross-section is '
            . '<em>A</em>&nbsp;=&nbsp;π&nbsp;<em>d</em>²/4. In practice, every three AWG steps roughly '
            . 'double the copper area, and six steps roughly double the diameter. The mm² column in the '
            . 'chart above already carries the ASTM&nbsp;B258 nominal values, so you can read the '
    ...
    [
        ...
        '<p>Because it is a marketing label, not a measurement. True 4&nbsp;AWG is 21.15&nbsp;mm² of '
            . 'copper. Sellers of flexible silicone wire frequently round up to the nearest metric size, '
            . 'or quote the outside diameter over the insulation instead of the conductor. Always size '
            . 'from the strand count and strand diameter on the datasheet — that is what the calculator on '
            . 'this page asks for — and treat any headline mm² figure as unverified until it matches.</p>',
    ],
    [
        ...
        '<p>Not by itself. Ampacity follows the total copper cross-section, so 1&nbsp;650 strands of '
            . '0.08&nbsp;mm carry the same current as a solid conductor of the same area. What fine '
            . 'stranding buys you is flexibility, vibration and flex-cycle life, and easier routing in '
            . 'tight enclosures. At DC and mains frequencies the skin effect is negligible at these sizes, '
            . 'so stranding gives no measurable current bonus.</p>',
    ],
    [
        'How do I calculate DC voltage drop?',
        '<p>Voltage drop is the load current times the resistance of the full circuit — out and '
            . 'back. With copper resistivity ρ&nbsp;=&nbsp;0.0175&nbsp;Ω·mm²/m, the drop is '
            . 'ΔU&nbsp;=&nbsp;<em>I</em>&nbsp;×&nbsp;ρ&nbsp;×&nbsp;2<em>L</em>&nbsp;/&nbsp;<em>A</em>, '
            . 'where <em>L</em> is the one-way run length. Most low-voltage DC systems aim to keep the '
            . 'drop under 3&nbsp;% of system voltage; on a 12&nbsp;V circuit that is only 0.36&nbsp;V, '
            . 'which is why long 12&nbsp;V runs are usually sized by voltage drop rather than by '
            . 'ampacity.</p>',
    ],
    [
        'How much should I derate for bundled conductors?',
        '<p>Bundling traps heat, so every conductor in the group carries less. Common factors are '
            . '80&nbsp;% for 4–6 current-carrying conductors, 70&nbsp;% for 7–9, 50&nbsp;% for 10–20, '
            . '45&nbsp;% for 21–30, 40&nbsp;% for 31–40 and 35&nbsp;% beyond that. Ambient temperature '
            . 'derates on top of grouping, and the two multiply. Conductors that never carry current at '
            . 'the same time, and neutrals in a balanced circuit, usually do not count towards the '
            . 'group.</p>',
    ],
    [
        'Can I use this chart for aluminium wire?',
        '<p>Yes — switch the conductor material and every column recalculates. Aluminium at '
            . '61.2&nbsp;% IACS carries about 78&nbsp;% of copper\'s current at the same cross-section, '
            . 'because ampacity follows the square root of conductivity. Matching a copper conductor '
            . 'therefore takes 1.64 times the area, roughly two AWG sizes up. The arithmetic is the easy '
            . 'part: aluminium creeps under clamping pressure, oxidises on contact with air and expands '
            . 'more than copper when hot, so it needs terminals listed for aluminium, anti-oxidant '
            . 'compound and a torque wrench. Copper-clad aluminium behaves the same electrically while '
            . 'terminating like copper, and copper-clad steel at 30&nbsp;% IACS is for strength and RF, '
            . 'not for power.</p>',
    ],
    [
        ...
        '<p>Not meaningfully at mains frequency. Alternating current crowds toward the conductor '
            ...
            . '60&nbsp;Hz, while a 0&nbsp;AWG conductor has a radius of just 4.1&nbsp;mm. By '
            . 'IEC&nbsp;60287 the resistance rise works out at 0.08&nbsp;% for 0&nbsp;AWG and under '
            . '0.01&nbsp;% below 4&nbsp;AWG, so the same table serves DC, single-phase and three-phase. '
            . 'What the current type really changes is the voltage drop: two conductors out and back for '
            . 'DC and single-phase, √3 line-to-line for three-phase, with the power factor on top. Skin '
            . 'effect only becomes worth counting in the hundreds of hertz, which is why the calculator '
            . 'asks for a frequency.</p>',
    ],
    [
        'Is this chart the same as NEC or IEC ampacity?',
        '<p>No. This is a reference for flexible, fine-stranded tinned copper lead wire with '
            . 'silicone insulation, of the kind used in equipment wiring, battery leads, robotics and RC. '
            . 'Building installations are governed by NEC&nbsp;310 tables in North America and '
            . 'IEC&nbsp;60364-5-52 in Europe, which assume different insulation, installation methods and '
            . 'correction factors. Use those codes for fixed wiring, and use this chart for equipment and '
            . 'appliance wiring.</p>',
    ],
];
...
$EN_I18N = [
    ...
    'insulationNote' => '<strong>{name}, {c} °C.</strong> Conductor rating {c} °C, ampacity ×{factor} against the '
        . 'published 200 °C silicone data.',
    'materialNote' => '<strong>{name}.</strong> {iacs}% IACS, ρ = {rho} Ω·mm²/m, ampacity ×{factor} against '
        . 'annealed copper.',
    'unitHighTemp' => '°C / A',
    'uHz' => 'Hz',
    ...
    'modeNote' => [
        'dc' => '<strong>Direct current.</strong> Voltage drop is counted over the full loop, out and back.',
        'ac1' => '<strong>Single-phase AC.</strong> Two loaded conductors, same as DC, so the only difference '
            . 'is skin and proximity loss: 0.43% on 0 AWG at 60 Hz, 0.07% at 4 AWG, nothing measurable '
            . 'below that. Raise the frequency to see it — 0 AWG loses 8% by 400 Hz.',
        'ac3' => '<strong>Three-phase AC.</strong> Three current-carrying conductors share the cable, so the '
            . 'in-cable columns carry the IEC 60364-5-52 factor of 0.915 against the 2-conductor case. '
            . 'Drop is √3·I·R·L line-to-line over the one-way run.',
    ],
    'tableModeNote' => [
        'dc' => 'These figures are for direct current.',
        'ac1' => 'Single-phase has two loaded conductors, exactly like DC, so no conductor-count derate '
            . 'applies. Only skin and proximity loss separate the two, and at 60 Hz that is 0.43% on 0 AWG '
            . 'and less than 0.07% below 4 AWG — which is why most rows read identically to DC.',
        'ac3' => 'The in-cable columns carry the IEC 60364-5-52 factor of 0.915 for three loaded conductors '
            . 'instead of two. The free-air columns describe a single isolated conductor and are unchanged.',
    ],
    'msgSkinRange' => 'Above roughly 1 kHz on the largest gauges the skin-effect fit leaves its validated range, '
        . 'so treat the AC resistance as indicative only.',
    'uAmp' => 'A',
    'uVolt' => 'V',
    'uWatt' => 'W',
    ...
    'msgDanger' => 'The selected ambient exceeds one of the reference ranges. Review both temperature results '
        . 'before choosing a lower thermal target or a different wire system.',
    'msgReady' => 'Ampacity is calculated. Add the optional load current, system voltage, and one-way length '
        . 'to check load margin and voltage drop.',
    'msgAboveBoth' => '<strong>{i} A</strong> is above both the 60°C reference of <strong>{a60} A</strong> and the '
        . '200°C reference of <strong>{a200} A</strong>. Increase the conductor size, improve cooling, '
        . 'or reduce the load.',
    'msgAbove60' => '<strong>{i} A</strong> is above the conservative 60°C reference of <strong>{a60} '
        . 'A</strong>, but within the 200°C limit of <strong>{a200} A</strong>. Confirm the actual '
        . 'wire, terminal, fuse, enclosure, and local-code limits before use.',
    'msgWithin' => '<strong>{i} A</strong> is within both temperature references. The 60°C value remains the '
        . 'conservative everyday design baseline; confirm the terminal, fuse, enclosure, and '
        . 'manufacturer limits before final use.',
];;
...
{$faq}
            ...
            <section id="source">
              ...
              <ul>
                <li><strong>src/build.php</strong> — the ampacity table, the copy, the FAQ and the JSON-LD</li>
...
$EN = [
    ...
    'OG_LOCALE' => 'en_US',
    'OG_LOCALE_ALT' => 'uk_UA',
    'CANONICAL' => 'https://66ton99.org.ua/awg-to-amps',
    'TITLE' => 'AWG to Amps Chart &amp; Wire Gauge Ampacity Calculator',
    'OG_TITLE' => 'AWG to Amps Chart &amp; Wire Gauge Ampacity Calculator',
    'DESC' => 'AWG to amps chart for 30–0 AWG copper wire: real mm² cross-section, 60 °C and 200 °C '
        . 'ampacity, plus a free calculator for strands, derating and voltage drop.',
    'OG_IMAGE' => 'https://66ton99.org.ua/og-awg-to-amps.png',
    ...
    'LANG_LINKS' => '<span aria-current="true" lang="en">EN</span><a href="/uk/awg-to-amps" hreflang="uk" '
        . 'lang="uk">УК</a>',
    ...
    'HERO_P' => 'A working reference for flexible, fine-stranded lead wire, from 30 AWG to 0 AWG. Read '
        . 'current capacity by real conductor cross-section in mm², then use the calculator to check a '
        . 'custom strand construction, conductor material, insulation temperature rating, bundle '
        . 'derating and voltage drop on DC or AC.',
    ...
    'P_TABLE' => 'Power sizes from 30 AWG through 0 AWG, including the 4 AWG / 25 mm² market-label case. '
        . 'Select any gauge to load a representative 0.08 mm fine-strand construction into the '
        . 'calculator.',
    'COUNT' => '17 gauges',
    'TABLE_CAPTION' => 'AWG to amps: copper cross-section in mm², reference ampacity at 60 °C and 200 °C, conductor '
        . 'resistance and the maximum run at 3% voltage drop, for DC or AC',
    'TH_GAUGE' => 'Gauge',
    ...
    'TABLE_NOTE' => '<strong>Conditions:</strong> copper lead wire, 30°C ambient. The 60°C columns are the '
        . 'conservative everyday reference. The 200°C columns are high-temperature conductor limits, '
        . 'not touch-safe or connector-safe operating targets. More than three conductors require '
        . 'further derating. AWG 24–30 rows are indicative for fine-stranded silicone wire and must be '
        . 'checked against the exact cable datasheet.',
    // MATERIALS: computed
    // DEFAULT_MATERIAL: computed
    // INSULATIONS: computed
    // INSULATION_OPTIONS: computed
    'INSULATION_LABEL' => 'Insulation',
    'TEMP_LABEL' => '°C',
    'INSULATION_NOTE' => '<strong>Silicone, 200 °C.</strong> Conductor rating 200 °C, ampacity ×1.000 against the '
        . 'published silicone data.',
    // MATERIAL_OPTIONS: computed
    'MATERIAL_LABEL' => 'Conductor material',
    'IACS_LABEL' => '% IACS',
    'MATERIAL_NOTE_DEFAULT' => '<strong>Copper, tinned.</strong> 96% IACS, ρ = 0.01823 Ω·mm²/m, ampacity ×0.98 against '
        . 'annealed copper.',
    ...
    'P_CALC' => 'Count strands, enter their diameter, and get the ampacity of that exact cross-section at '
        . 'both temperature references — interpolated between the table rows, not rounded to one.',
    'L_STRANDS' => 'Strand count',
    'L_DIAM' => 'Strand diameter / mm',
    'L_INSTALL' => 'Installation / grouping',
    // INSTALL_OPTIONS: computed
    ...
    'SOURCE_NOTE' => 'Small-gauge entries are cross-checked against <a '
        . 'href="https://www.amphenol.co.jp/military/techinfo/CurrentCarryingCapacity.html" '
        . 'target="_blank" rel="noopener noreferrer">Amphenol Japan cable current-capacity data</a>. '
        . 'AWG areas follow <a href="https://store.astm.org/b0258-18r26.html" target="_blank" '
        . 'rel="noopener noreferrer">ASTM B258</a>. Final sizing must follow the exact wire, terminal, '
        . 'fuse, enclosure, and local-code requirements.',
    // CONTENT: computed
    ...
    'INSTALL_OPTIONS' => build_options($EN_INSTALL),
    'CONTENT' => $EN_CONTENT,
    'QUICKNAV' => quicknav($EN_QUICKNAV),
    'I18N' => json_py($EN_I18N),
...
$UK_FAQ = [
    [
        ...
        '<p>Для лудженої мідної жили за температури довкілля 30&nbsp;°C консервативна колонка '
            . '60&nbsp;°C у таблиці вище дає приблизно 7,8&nbsp;А для 18&nbsp;AWG, 13,7&nbsp;А для '
            . '14&nbsp;AWG, 25,5&nbsp;А для 10&nbsp;AWG, 34,3&nbsp;А для 8&nbsp;AWG, 60,7&nbsp;А для '
            . '4&nbsp;AWG і 109,7&nbsp;А для 0&nbsp;AWG за умови до трьох струмопровідних жил у кабелі. '
            . 'Одинарний провідник у вільному повітрі охолоджується краще й витримує більше. Силіконовий '
            . 'дріт із робочою температурою 200&nbsp;°C витримує приблизно вдвічі більше, але лише якщо '
            . 'кожна клема, запобіжник і конектор у колі також розраховані на таку температуру.</p>',
    ],
    [
        ...
        '<p>Обидві описують ту саму мідь. Різниця в тому, наскільки сильно ви дозволяєте провіднику '
            . 'нагрітися. Колонки 60&nbsp;°C — це щоденна проєктна база: дріт залишається безпечним на '
            . 'дотик, а ізоляція, клеми та сусідні матеріали працюють у комфортному режимі. Колонки '
            . '200&nbsp;°C — це тепловий ліміт самої силіконової ізоляції, тобто струм, за якого провідник '
            . 'досягає 200&nbsp;°C. Друге число — це стеля для коротких добре провітрюваних ділянок, а не '
            . 'проєктна ціль.</p>',
    ],
    [
        'Як перевести AWG у мм²?',
        '<p>AWG — логарифмічна шкала, тому тут працює формула, а не проста пропорція. Діаметр '
            . 'провідника в міліметрах: <em>d</em>&nbsp;=&nbsp;0,127&nbsp;×&nbsp;92<sup>(36−AWG)/39</sup>, '
            . 'а переріз — <em>A</em>&nbsp;=&nbsp;π&nbsp;<em>d</em>²/4. На практиці кожні три кроки AWG '
            . 'приблизно подвоюють площу міді, а шість кроків — діаметр. Колонка мм² у таблиці вже містить '
            . 'номінальні значення за ASTM&nbsp;B258, тож переведення можна зчитати просто з таблиці.</p>',
    ],
    [
        'Чому дріт 25&nbsp;мм² продають як 4&nbsp;AWG?',
        '<p>Бо це маркетингове маркування, а не вимір. Справжній 4&nbsp;AWG — це 21,15&nbsp;мм² '
            . 'міді. Продавці гнучкого силіконового дроту часто округлюють до найближчого метричного '
            . 'розміру або вказують зовнішній діаметр по ізоляції замість перерізу жили. Завжди рахуйте за '
            . 'кількістю жилок і діаметром жилки з даташита — саме це просить калькулятор на цій сторінці '
            . '— і вважайте заявлену цифру мм² неперевіреною, доки вона не збіжиться.</p>',
    ],
    [
        'Чи збільшує більша кількість жилок допустимий струм?',
        '<p>Сама собою — ні. Допустимий струм визначається сумарним перерізом міді, тому 1650 жилок '
            . 'по 0,08&nbsp;мм пропускають той самий струм, що й суцільний провідник такої ж площі. Тонке '
            . 'багатожильне плетіння дає гнучкість, стійкість до вібрації та згинів і зручність '
            . 'прокладання у тісних корпусах. На постійному струмі та мережевих частотах скін-ефект на цих '
            . 'перерізах знехтуваний, тож приросту струму багатожильність не дає.</p>',
    ],
    [
        'Як порахувати падіння напруги на постійному струмі?',
        '<p>Падіння напруги — це струм навантаження, помножений на опір усього кола, туди й назад. '
            . 'За питомого опору міді ρ&nbsp;=&nbsp;0,0175&nbsp;Ом·мм²/м падіння дорівнює '
            . 'ΔU&nbsp;=&nbsp;<em>I</em>&nbsp;×&nbsp;ρ&nbsp;×&nbsp;2<em>L</em>&nbsp;/&nbsp;<em>A</em>, де '
            . '<em>L</em> — довжина траси в один бік. Більшість низьковольтних систем постійного струму '
            . 'тримають падіння в межах 3&nbsp;% від напруги живлення; для кола 12&nbsp;В це лише '
            . '0,36&nbsp;В — саме тому довгі 12-вольтові траси зазвичай рахують за падінням напруги, а не '
            . 'за допустимим струмом.</p>',
    ],
    [
        'Наскільки знижувати струм для жил у пучку?',
        '<p>Пучок затримує тепло, тож кожна жила в групі витримує менше. Типові коефіцієнти: '
            . '80&nbsp;% для 4–6 струмопровідних жил, 70&nbsp;% для 7–9, 50&nbsp;% для 10–20, 45&nbsp;% '
            . 'для 21–30, 40&nbsp;% для 31–40 і 35&nbsp;% далі. Температура довкілля знижує струм '
            . 'додатково, і два коефіцієнти перемножуються. Жили, що ніколи не працюють одночасно, а також '
            . 'нейтралі у збалансованому колі, зазвичай до групи не зараховуються.</p>',
    ],
    [
        ...
        '<p>Так — перемкніть матеріал жили, і всі колонки перерахуються. Алюміній із провідністю '
            . '61,2&nbsp;% IACS несе близько 78&nbsp;% струму міді того самого перерізу, бо допустимий '
            . 'струм змінюється як корінь із провідності. Тому щоб зрівнятися з мідною жилою, потрібно у '
            . '1,64 раза більше площі — приблизно два кроки AWG угору. Арифметика тут найпростіше: '
            . 'алюміній тече під тиском затиску, окислюється на повітрі й розширюється від нагріву '
            . 'сильніше за мідь, тож потребує клем, сертифікованих для алюмінію, антиоксидної пасти й '
            . 'динамометричного ключа. Мідно-алюмінієвий провід поводиться електрично так само, але '
            . 'з\'єднується як мідь, а мідно-сталевий із 30&nbsp;% IACS призначений для міцності та високих '
            . 'частот, а не для потужності.</p>',
    ],
    [
    ...
    [
        ...
        '<p>Ні. Це довідник для гнучкого тонкожильного лудженого мідного дроту з силіконовою '
            . 'ізоляцією — того, що використовують у внутрішньому монтажі обладнання, акумуляторних '
            . 'перемичках, робототехніці та RC-моделях. Стаціонарні електроустановки регулюються таблицями '
            . 'NEC&nbsp;310 у Північній Америці та IEC&nbsp;60364-5-52 у Європі, які виходять з іншої '
            . 'ізоляції, способів прокладання та поправкових коефіцієнтів. Для стаціонарної проводки '
            . 'користуйтеся нормами, а цю таблицю застосовуйте для монтажу обладнання.</p>',
    ],
];
...
$UK_QUICKNAV = [
    [
        '#chart',
        'Таблиця AWG в ампери',
    ],
    [
        '#calculator',
        'Калькулятор перерізу',
    ],
    [
        '#how-to-read',
        'Як читати таблицю',
...
$UK_I18N = [
    ...
    'insulationNote' => '<strong>{name}, {c} °C.</strong> Робоча температура жили {c} °C, струм ×{factor} відносно '
        . 'опублікованих даних для силікону 200 °C.',
    'materialNote' => '<strong>{name}.</strong> {iacs}% IACS, ρ = {rho} Ом·мм²/м, струм ×{factor} відносно '
        . 'відпаленої міді.',
    'unitHighTemp' => '°C / А',
    'uHz' => 'Гц',
    ...
    'modeNote' => [
        ...
        'ac1' => '<strong>Однофазний змінний.</strong> Жили дві, тож падіння все ще рахується туди й назад, '
            . 'але з поправкою на cos φ. Допустимий струм такий самий, як для постійного: на 50–60 Гц '
            . 'скін-ефект менший за 0,1 % навіть для 0 AWG.',
        'ac3' => '<strong>Трифазний змінний.</strong> Падіння дорівнює √3·I·R·L між лініями на довжині в один '
            . 'бік, а три фазні жили — це рівно випадок «≤3 жили» із таблиці.',
    ],
    'tableModeNote' => [
        'dc' => 'Ці значення наведено для постійного струму.',
        'ac1' => 'Ці значення без змін діють і для однофазного змінного струму 50–60 Гц: глибина скін-шару в '
            . 'міді — близько 9,4 мм на 50 Гц, що набагато більше за радіус 4,1 мм навіть у жили 0 AWG, '
            . 'тож приріст опору лишається меншим за 0,1 %.',
        'ac3' => 'Колонки «у кабелі» знижено на коефіцієнт 0,915 за IEC 60364-5-52: три струмопровідні жили '
            . 'гріють кабель сильніше за дві. Колонки вільного повітря описують одну жилу і не змінюються.',
    ],
    'msgSkinRange' => 'Понад приблизно 1 кГц на найбільших калібрах формула скін-ефекту виходить за межі '
        . 'перевіреного діапазону, тож опір на ЗС вважайте орієнтовним.',
    'uAmp' => 'А',
    'uVolt' => 'В',
    'uWatt' => 'Вт',
    ...
    'msgDanger' => 'Задана температура довкілля виходить за межі одного з довідкових діапазонів. Перегляньте '
        . 'обидва температурні результати, перш ніж обирати нижчий тепловий режим або іншу кабельну '
        . 'систему.',
    'msgReady' => 'Допустимий струм розраховано. Додайте струм навантаження, напругу системи та довжину в один '
        . 'бік, щоб перевірити запас і падіння напруги.',
    'msgAboveBoth' => '<strong>{i} А</strong> перевищує і режим 60°C (<strong>{a60} А</strong>), і режим 200°C '
        . '(<strong>{a200} А</strong>). Збільште переріз, покращте охолодження або зменште '
        . 'навантаження.',
    'msgAbove60' => '<strong>{i} А</strong> перевищує консервативний режим 60°C (<strong>{a60} А</strong>), але '
        . 'вкладається в межу 200°C (<strong>{a200} А</strong>). Перевірте конкретний дріт, клеми, '
        . 'запобіжник, корпус і місцеві норми перед застосуванням.',
    'msgWithin' => '<strong>{i} А</strong> вкладається в обидва температурні режими. Значення 60°C залишається '
        . 'консервативною щоденною базою; перед остаточним застосуванням перевірте клеми, запобіжник, '
        . 'корпус і обмеження виробника.',
];;
...
{$faq}
            ...
            <section id="source">
              ...
              <ul>
                <li><strong>src/build.php</strong> — таблиця струмів, тексти, FAQ і розмітка JSON-LD</li>
...
$UK = [
    ...
    'OG_LOCALE' => 'uk_UA',
    'OG_LOCALE_ALT' => 'en_US',
    'CANONICAL' => 'https://66ton99.org.ua/uk/awg-to-amps',
    'TITLE' => 'Таблиця AWG в ампери та калькулятор перерізу дроту',
    'OG_TITLE' => 'Таблиця AWG в ампери та калькулятор перерізу дроту',
    'DESC' => 'Таблиця AWG в ампери для мідного дроту 30–0 AWG: реальний переріз у мм², струм за 60 °C і '
        . '200 °C, калькулятор жилок, поправок на пучок і падіння напруги.',
    'OG_IMAGE' => 'https://66ton99.org.ua/og-awg-to-amps-uk.png',
    ...
    'LANG_NAV_LABEL' => 'Мова',
    'LANG_LINKS' => '<a href="/awg-to-amps" hreflang="en" lang="en">EN</a><span aria-current="true" '
        . 'lang="uk">УК</span>',
    ...
    'HERO_P' => 'Робочий довідник для гнучкого тонкожильного монтажного дроту, від 30 AWG до 0 AWG. Дивіться '
        . 'допустимий струм за реальним перерізом жили в мм², а потім перевіряйте в калькуляторі '
        . 'власну конструкцію жилок, матеріал жили, температурний клас ізоляції, поправку на пучок і '
        . 'падіння напруги на постійному чи змінному струмі.',
    ...
    'COUNT' => '17 калібрів',
    'TABLE_CAPTION' => 'AWG в ампери: переріз міді в мм², довідковий допустимий струм за 60 °C і 200 °C, опір жили '
        . 'та максимальна траса за падіння 3%, для постійного або змінного струму',
    'TH_GAUGE' => 'Калібр',
    ...
    'TABLE_NOTE' => '<strong>Умови:</strong> мідний монтажний дріт, довкілля 30°C. Колонки 60°C — консервативний '
        . 'щоденний орієнтир. Колонки 200°C — граничні температури провідника, а не безпечні для '
        . 'дотику чи для клем робочі значення. Понад три жили потребують додаткового зниження струму. '
        . 'Рядки AWG 24–30 є орієнтовними для тонкожильного силіконового дроту й потребують звірки з '
        . 'даташитом конкретного кабелю.',
    // MATERIALS: computed
    // DEFAULT_MATERIAL: computed
    // INSULATIONS: computed
    // INSULATION_OPTIONS: computed
    ...
    'TEMP_LABEL' => '°C',
    'INSULATION_NOTE' => '<strong>Силікон, 200 °C.</strong> Робоча температура жили 200 °C, струм ×1,000 відносно '
        . 'опублікованих даних для силікону.',
    // MATERIAL_OPTIONS: computed
    'MATERIAL_LABEL' => 'Матеріал жили',
    'IACS_LABEL' => '% IACS',
    'MATERIAL_NOTE_DEFAULT' => '<strong>Мідь, луджена.</strong> 96% IACS, ρ = 0,01823 Ом·мм²/м, струм ×0,98 відносно '
        . 'відпаленої міді.',
    ...
    'P_CALC' => 'Вкажіть кількість жилок і їхній діаметр — і отримайте допустимий струм саме цього перерізу '
        . 'за обома температурними режимами: з інтерполяцією між рядками таблиці, а не з округленням '
        . 'до рядка.',
    ...
    'SOURCE_NOTE' => 'Значення для малих калібрів звірені з <a '
        . 'href="https://www.amphenol.co.jp/military/techinfo/CurrentCarryingCapacity.html" '
        . 'target="_blank" rel="noopener noreferrer">даними Amphenol Japan щодо струмового '
        . 'навантаження кабелів</a>. Площі AWG відповідають <a '
        . 'href="https://store.astm.org/b0258-18r26.html" target="_blank" rel="noopener '
        . 'noreferrer">ASTM B258</a>. Остаточний вибір перерізу має враховувати конкретний дріт, '
    ...
    'INSTALL_OPTIONS' => build_options($UK_INSTALL),
    'CONTENT' => $UK_CONTENT,
    'QUICKNAV' => quicknav($UK_QUICKNAV),
    'I18N' => json_py($UK_I18N),
 ...
 * @param list<array{string,string}> $faqItems
 */
function build_jsonld(array $cfg, array $faqItems): string
{
    $url = $cfg['CANONICAL'];
    $lang = $cfg['LANG'];
    $title = str_replace('&amp;', '&', $cfg['TITLE']);
    ...
    foreach ($faqItems as [$q, $a]) {
        $questions[] = [
            '@type' => 'Question',
            'name' => strip_tags_text($q),
            'acceptedAnswer' => ['@type' => 'Answer', 'text' => strip_tags_text($a)],
        ];
    }
    ...
    return json_py(['@context' => 'https://schema.org', '@graph' => $graph], indent: 2);
}
 ...
 * @param list<array{string,string}> $faqItems
 */
function render(array $cfg, array $faqItems, string $outPath): void
{
    $values = $cfg;
    $values['TBODY'] = build_tbody($cfg['LANG']);
    $values['JSONLD'] = build_jsonld($cfg, $faqItems);
    // FEATURES is a list consumed by the JSON-LD above; it has no page token.
    unset($values['FEATURES']);

    render_template(file_get_contents(HERE . '/template.html'), $values, $outPath, OUT);
}


/**
 * @param array<string,mixed>        $en
 * @param list<array{string,string}> $enFaq
 * @param array<string,mixed>        $uk
 * @param list<array{string,string}> $ukFaq
 */
function main(array $en, array $enFaq, array $uk, array $ukFaq): void
{
    if (!is_dir(OUT)) {
        mkdir(OUT, 0o777, true);
    }
    render($en, $enFaq, OUT . '/_pages/awg-to-amps.html');
    render($uk, $ukFaq, OUT . '/_pages/uk-awg-to-amps.html');
    ...
    $altLinks = static function (string $enHref, string $ukHref, int $indent = 4): string {
        $pad = str_repeat(' ', $indent);

        return $pad . '<xhtml:link rel="alternate" hreflang="en" href="' . $enHref . "\"/>\n"
            . $pad . '<xhtml:link rel="alternate" hreflang="uk" href="' . $ukHref . "\"/>\n"
            . $pad . '<xhtml:link rel="alternate" hreflang="x-default" href="' . $enHref . '"/>';
    };

    $homeEn = BASE . '/';
    $homeUk = BASE . '/uk';
    $homeAlts = $altLinks($homeEn, $homeUk);
    $pageAlts = $altLinks(EN_URL, UK_URL);
    $enUrl = EN_URL;
    $ukUrl = UK_URL;
    ...
    write_file(OUT . '/sitemap.xml', $sitemap);
    echo "wrote sitemap.xml\n";
}


main($EN, $EN_FAQ, $UK, $UK_FAQ);
